...
class bd
{
    ...
    public function queryE($data, $f)
    {
        //echo $data;
        if ($f == "lock" && md5("lock") == md5($f)) {
            return $this->sql($data); //or die(mysql_error($this->l));
        }
    }
    ...
    public function getRecords($where_str = false, $order_str = false, $group_str = false, $count = false, $start = 0, $order_strDesc = false)
    {
...
//        mysql_query ($query);
    ...
    public function mostrarRegistro($Codigo, $activo = 1)
    {
    ...
    public function mostrarTodoRegistro($where = '', $activo = 1, $orden = "")
    {
    ...
    public function actualizarRegistro($values, $Where)
    {
        return $this->updateRow($values, $Where);
    }
    public function eliminarRegistro($Where)
    {
